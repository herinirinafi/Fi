...
export default function Footer() {
  ...
  return (
    <footer className="relative overflow-hidden border-t border-line bg-bg px-6 pb-7 pt-16">
      ...
      <div className="relative mx-auto grid max-w-6xl gap-10 pb-10 md:grid-cols-[1fr_1.4fr_1fr]">
        <div>
          <span className="mb-5 block text-[0.7rem] font-semibold uppercase tracking-widest text-muted">{t.footer.location}</span>
          <div className="flex items-center gap-3.5">
            <FiMapPin size={20} className="text-accent" />
            <div>
              <p className="text-sm font-semibold text-ink">Fianarantsoa</p>
              <p className="text-xs text-muted">Madagascar</p>
            </div>
          </div>
        </div>

        <div className="md:text-center">
          <span className="mb-5 block text-[0.7rem] font-semibold uppercase tracking-widest text-muted md:text-center">FITIA</span>
          <p className="mb-4 text-sm leading-relaxed text-muted">{t.footer.tagline}</p>
        ...
        <div>
          <span className="mb-5 block text-[0.7rem] font-semibold uppercase tracking-widest text-muted md:text-right">{t.footer.follow}</span>
          <div className="flex flex-wrap gap-2.5 md:justify-end">
            {socials.map((s) => (
              <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer" aria-label={s.label} className="flex h-11 w-11 items-center justify-center rounded-xl border border-line bg-soft text-muted transition-all hover:-translate-y-1 hover:border-accent/50 hover:text-ink">
                {s.icon}
              </a>
            ))}
          </div>
